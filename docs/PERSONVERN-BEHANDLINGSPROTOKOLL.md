...
| `listing_images`, `listing_360_frames` (metadata i Postgres)   | Annonseinnhold                                | Avtale   | Cloudflare R2 + Postgres-metadata                                                          | Postgres-metadata slettes når annonsen slettes (kaskade); R2-objektene ryddes av slettekøen (§ 2.1)                                                                                                           |
| Annonsebilder og 360-frames (binære filer)                     | Vise brukere aktivt annonserte kjøp/salg      | Avtale   | Cloudflare R2 (`kaupet-bilder` / staging: `kaupet-bilder-staging`), bak `bilder.kaupet.no` | Slettes automatisk når annonsen slettes: en trigger legger annonsens R2-prefiks i slettekøen, og opprydningsjobben sletter objektene (§ 2.1). Enkeltbilder kan i tillegg slettes manuelt i annonseredigering. |
| Avatarer og organisasjonslogoer (binære filer)                 | Brukerprofil og bedriftspresentasjon          | Avtale   | Cloudflare R2 (`kaupet-bilder` / staging: `kaupet-bilder-staging`), bak `bilder.kaupet.no` | Slettes når bruker bytter avatar/logo (via `deleteObject`). Ved kontosletting legges brukerens avatarprefiks i slettekøen og objektene fjernes (§ 2.1). Organisasjonslogoer ryddes foreløpig kun ved bytte.   |
...
### 2.1 Opprydning av binærfiler i R2

Postgres kan ikke selv nå R2, så kaskadesletting av en metadatarad fjerner
ikke selve filen. Det håndteres av en slettekø
(`supabase/migrations/20260918230000_r2_delete_queue.sql`):

- Triggere på `listings`, `conversations` og `profiles` legger den slettede
  radens R2-prefiks i `r2_delete_queue`, i samme transaksjon som slettingen.
  Nøklene er partisjonert på eier-id, så ett prefiks dekker alle filene —
  annonsebilder, thumbnails og 360-frames ligger alle under `{listingId}/`.
- Triggeren på `conversations` fyrer også når samtalen forsvinner via kaskade
  fra annonsen, så meldingsvedlegg fanges uansett hvordan slettingen startet.
- Kontosletting (`purge_expired_accounts`) sletter annonsene — som fanges av
  triggeren over — og anonymiserer profilraden. Avatarfilen fanges på
  overgangen til `deleted_at`.
- En pg_cron-jobb (`r2-cleanup-hourly`) kaller
  `/api/public/r2/cleanup`, som sletter objektene og fjerner køraden først
  når slettingen lyktes. Mislykkede forsøk blir stående med `attempts` og
  `last_error`, og prøves på nytt neste time.

Organisasjonslogoer ryddes foreløpig kun når logoen byttes; sletting av en
hel organisasjon er ikke dekket av køen.
...
| Meldingsvedlegg (binære filer)                                    | Privat korrespondanse mellom kjøper og selger    | Avtale                       | Cloudflare R2 (`kaupet-vedlegg` / staging: `kaupet-vedlegg-staging`) — privat bucket, lesing via presignerte S3-URL-er med 1 times utløpstid | Slettes automatisk når samtalen slettes — også når den forsvinner via kaskade fra annonsen: en trigger legger samtalens R2-prefiks i slettekøen (§ 2.1).             |
...
| Faktisk sletting (`purge_expired_accounts`)   | Utføre sletting                          | Avtale                         | —                 | 7 dager etter forespørsel: selgerens egne salgs- **og** kjøpsønske-annonser slettes, profil anonymiseres (`display_name` → «Slettet bruker», `avatar_url` settes til NULL, og selve bildefilen i R2 ryddes av slettekøen, se § 2.1), `auth.users`-raden (e-post, innloggingsdata) slettes permanent |
...
| All trafikk til/fra kaupet.no                                                     | Drift, sikkerhet (WAF/DDoS)        | Berettiget interesse | Cloudflare (driftsplattform, Workers-runtime)                       | Cloudflare DPA (SCC)                                                                    |
| Bildelagring (annonser, avatarer, organisasjonslogoer, 360-frames)                | Lagre og levere annonsebilder      | Avtale               | Cloudflare R2                                                       | Cloudflare DPA (SCC). Offentlig bucket for publiserte annonser, presignerte URL-er      |
| Meldingsvedlegg (privat korrespondanse)                                           | Lagre vedlegg i private meldinger  | Avtale               | Cloudflare R2 (privat bucket)                                       | Cloudflare DPA (SCC). Privat bucket, lesing via presignerte S3-URL-er med tidsgrense    |
| Database, autentisering                                                           | Kjernetjenesten                    | Avtale               | Supabase                                                            | Supabase DPA; region bekreftes i Supabase-dashbordet (ikke verifisert fra dette repoet) |
