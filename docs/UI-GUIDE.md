...
- **Statuslinje-stripe:** `styles.css` legger en fast, ugjennomsiktig stripe over statuslinjas høyde på alle native flater (`html.native body::before`, `z-index: 20`). Den finnes fordi `.pt-safe` kun reserverer plass øverst i seksjonen som bruker den, ved scrollposisjon 0 — under edge-to-edge Android dekker scrollet inn lesbart innhold rett under klokka og statusikonene når brukeren scroller fra andre steder på siden. Innhold som er ment å gå under statuslinja (helbilde, kart, kamera) må ligge over `z-index: 20` for å unngå stripen; eksisterende sticky-headere (`NativePageHeader` z-30, `SiteHeader` z-40) og alle overlays (`z-[10000]`) ligger allerede over.
...
annonser ligger i
[NATIVE-ANNONSEOPPRETTELSE-KORTFLYT-PLAN.md](plans/NATIVE-ANNONSEOPPRETTELSE-KORTFLYT-PLAN.md).
Historikk fra den tidligere composer-konsolideringen ligger i
[ANNONSEOPPRETTELSE-UX-PLAN.md](plans/ANNONSEOPPRETTELSE-UX-PLAN.md).
