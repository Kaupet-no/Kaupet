            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={5} className="py-8 text-center">
                  <p className="text-sm text-destructive">
                    {formatErrorMessage(queryError, "Kunne ikke laste feilloggen")}
                  </p>
                </TableCell>
              </TableRow>
            ) : (data ?? []).length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="py-8">
                  <EmptyState
                    icon={AlertOctagon}
                    title="Ingen feil registrert"
                    description="Feil fra systemet vises her etter hvert som de oppstår."
