import { Hash, LayoutGrid, MapPin, Search as SearchIcon } from "lucide-react";
...
export function AppLanding() {
  ...
  const shortcutClass =
    "native-touch-target flex min-w-0 flex-1 flex-col items-center gap-2 border-b border-border px-3 py-4 text-center text-sm font-medium transition active:opacity-70";
  ...
  return (
    <div className="pb-3">
      ...
      >
        <h1 className="mb-5 text-center font-display text-2xl tracking-tight">
        ...
        <button
          ...
          className={`native-touch-target mt-4 inline-flex max-w-full items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-sm transition active:opacity-80 ${
            hasLocation
      ...
      <div className="mb-3 flex items-stretch justify-center gap-3 px-5">
        <KaupetCodeDialog
          trigger={
            <button type="button" className={shortcutClass}>
        ...
        />
        <button type="button" onClick={() => openPanel("categories")} className={shortcutClass}>
          <span className="flex size-11 items-center justify-center text-primary">
      ...
      <section className="mt-5 pl-5" aria-labelledby="popular-heading">
