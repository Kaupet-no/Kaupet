  ...
  >
    ...
    {value.map((_, i) => (
      <SliderPrimitive.Thumb
        ...
        // after:-inset-3 pads the visible 24px thumb out to a 48px touch
        // target without inflating the track's own hit area between thumbs.
        className="relative block size-6 rounded-full border border-primary/50 bg-background shadow transition-colors after:absolute after:-inset-3 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50"
      />
    ))}
  </SliderPrimitive.Root>
));
