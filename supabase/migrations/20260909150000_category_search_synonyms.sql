-- Kategorisynonymer for kategorisøket i annonse-wizarden.
--
-- Søket i CategoryPicker matchet kun categories.name_nb, så en selger som
-- skrev "hodetelefon" fikk "Ingen kategorier funnet" selv om Elektronikk >
-- TV og lyd er det nærmeste stedet varen hører hjemme. Synonymene er et eget
-- felt og ikke en gjenbruk av search_examples: eksemplene mater den animerte
-- placeholderen på forsiden og er formulert som hele søkefraser, mens
-- synonymer er enkeltord vi vil treffe på uten å vise dem noe sted.

ALTER TABLE public.categories
  ADD COLUMN search_synonyms text[] NOT NULL DEFAULT '{}';

COMMENT ON COLUMN public.categories.search_synonyms IS
  'Alternative søkeord for kategorivelgeren (aldri vist i UI). Vedlikeholdes i /admin/kategorier.';

-- sync_categories_from_payload lister kategorikolonnene eksplisitt, så uten
-- at den også bærer den nye kolonnen ville search_synonyms blitt nullstilt i
-- produksjon ved hver staging -> prod-synk. Kroppen er ellers uendret fra
-- 20260909140000_simplify_category_flows.sql.
...
BEGIN
  ...
  INSERT INTO public.categories (
    ...
    search_examples, search_synonyms, title_example, is_hidden, created_at, updated_at
  )
  SELECT
    m.final_id,
    r->>'slug',
    ...
    COALESCE((SELECT array_agg(x) FROM jsonb_array_elements_text(r->'search_synonyms') x), '{}'),
    r->>'title_example',
